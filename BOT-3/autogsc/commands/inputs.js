...
module.exports = function registerInputs(bot) {
  ...
  bot.command('edit', (ctx) => {
    ...
    ctx.reply(
      ...
      `Paste isi baru — boleh multi-line, boleh multi-message.`,
      {
        ...
        reply_markup: {
          inline_keyboard: [[
            { text: '✅ Done (Simpan)', callback_data: 'run:done'   },
  ...
  bot.command('done', (ctx) => {
    ...
    try {
      ...
      let msg = `✅ *Saved!*\n\n`;
      msg += `📄 • File  : \`${inputs.INPUT_TYPES[savedType].label}\`\n`;
      ...
      ctx.reply(msg, {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              { text: '✅ Check Validasi', callback_data: 'run:check'             },
              { text: '📋 Show',           callback_data: `run:show ${savedType}` },
            ],
            [
  ...
  bot.on('document', async (ctx) => {
    ...
    try {
      ...
      if (dupCheck.hasDuplicates) {
        ...
        msg += `\n💡 Ganti entry duplikat lalu upload ulang.`;
        return ctx.reply(msg, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[
          { text: `📤 Upload Ulang`, callback_data: `run:upload ${uploading.type}` },
          { text: '❌ Cancel',        callback_data: 'run:cancel'                   },
        ]]}});
      }

      // AMAN, OVERWRITE FILE
      const uploadedType = uploading.type;
      state.uploadingState.delete(ctx.from.id);
      const result = inputs.write(uploadedType, data);
      let msg = `✅ *Uploaded!*\n\n`;
      msg += `📄 • File  : \`${inputs.INPUT_TYPES[uploadedType].label}\`\n`;
      ...
      ctx.reply(msg, {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              { text: '✅ Check Validasi',  callback_data: 'run:check'              },
              { text: '📋 Show',            callback_data: `run:show ${uploadedType}` },
            ],
            [
              { text: '⬅️ Input Files',    callback_data: 'menu:input'             },
              { text: '🏠 Menu Utama',     callback_data: 'menu:main'              },
